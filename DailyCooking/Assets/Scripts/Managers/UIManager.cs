...
// A NetworkBehaviour (with a NetworkObject, MainMenuScene) so the server can show UI on one
// client's screen: alerts and the option menu for the player who interacted. The option the
// player picks comes back here and is applied on the server with that player as the actor.
// ShowAlertMessage stays local and works without a session.

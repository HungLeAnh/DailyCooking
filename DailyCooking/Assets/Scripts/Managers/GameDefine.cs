...
public class GameDefine : MonoBehaviour
{
    ...
    /// <summary>
    /// Legacy hardcoded grid JSON. Prefer <see cref="DefaultGridConfigSO"/> asset at Resources/DefaultGridConfig.
    /// Kept for backward compatibility and as fallback if SO is missing.
    /// </summary>
    [Obsolete("Use DefaultGridConfigSO.gridArrayJson via GameDefine.GetDefaultGridJson() instead. Will be removed after data migration.")]
    ...
    private static DefaultGridConfigSO _cachedConfig;

    /// <summary>
    /// Resolves grid JSON from DefaultGridConfigSO (Resources/DefaultGridConfig) with fallback to legacy const.
    /// </summary>
    public static string GetDefaultGridJson()
    {
        if (_cachedConfig == null)
            _cachedConfig = Resources.Load<DefaultGridConfigSO>("DefaultGridConfig");
        if (_cachedConfig != null && !string.IsNullOrEmpty(_cachedConfig.gridArrayJson))
            return _cachedConfig.gridArrayJson;
        return GridArrayDataInit;
    }

    /// <summary>Clear cached SO (useful in editor after asset reimport).</summary>
    public static void ClearGridConfigCache() => _cachedConfig = null;
